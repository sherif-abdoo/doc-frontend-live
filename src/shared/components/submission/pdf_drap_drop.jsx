...
/**
 * PdfDropzone
 * - Calls your PRESIGN worker to get a signed PUT URL
 * - Uploads the PDF directly to R2
 * - Emits onUploadComplete(fileURL) with the final public URL (coming from the worker)
 */
    ...
    const uploadToCloudflare = async (file) => {
        try {
            ...
            if (typeof fileURL === "string") {
                onUploadComplete?.(fileURL); // ✅ always send the string URL
            }
        } catch (err) {
            console.error(err);
